<?php
// Assuming you have a connection to the database
$conn = mysqli_connect("localhost", "root", "", "signupform" , 3307);

if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}

// Check if the form is submitted
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Retrieve data from the form
    $patientId = isset($_POST['patient_id']) ? $_POST['patient_id'] : null;
    $doctorId = isset($_POST['doctor_id']) ? $_POST['doctor_id'] : null;
    $vaccineName = isset($_POST['vaccine_name']) ? $_POST['vaccine_name'] : null;
    $dose = isset($_POST['dose']) ? $_POST['dose'] : null;

    // Check if all required fields are provided
    if ($patientId !== null && $doctorId !== null && $vaccineName !== null && $dose !== null) {
        // Check if the patient exists
        $checkPatientSql = "SELECT * FROM `registrationp` WHERE id = $patientId";
        $checkPatientResult = mysqli_query($conn, $checkPatientSql);

        if ($checkPatientResult->num_rows > 0) {
            // Insert a new vaccine record if it doesn't exist
            $insertSql = "INSERT INTO `vaccine_bookings` (patient_id, doctor_id, vaccine_name, dose)
                          VALUES ($patientId, $doctorId, '$vaccineName', '$dose')";
            mysqli_query($conn, $insertSql);

            echo "Vaccine successfully booked!";

            // Optional: Redirect to a confirmation page
            // header("Location: confirmation.php");
            // exit();
        } else {
            echo "Patient not found.";
        }
    } else {
        echo "Please provide all required fields";
    }
}

// Close the database connection
$conn->close();
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Book Vaccine</title>
    <!-- Add your CSS styles or link to an external stylesheet here -->
    <style>
        /* Add your custom styles here */
        /* For example, you can style form elements or provide some layout */
        html {
            /* Add your custom styles for the html element here */
        }

        body {
            font-family: 'Arial', sans-serif;
            background-color: #f5f5f5;
            padding: 20px;
            background-color: #16a085;
            background-image: url('pngtree-health-medical-hospital-medicine-blue-banner-advertisement-image_177364.jpg');
            background-size: cover;
            background-position: center;
            display: flex;
            flex-direction: column;
            align-items: flex-start;
            justify-content: flex-start;
            height: 100vh;
        }

        form {
            max-width: 500px;
            margin: 0 auto;
            background-color: #fff;
            padding: 20px;
            border-radius: 8px;
            box-shadow: 1px 1px 10px rgba(0, 0, 0, 0.1);
        }

        input {
            width: 500px;
            padding: 10px;
            margin-bottom: 10px;
            box-sizing: border-box;
        }

        button {
            background-color: blue;
            color: white;
            padding: 10px;
            border: none;
            border-radius: 5px;
            cursor: pointer;
        }

        a {
            text-decoration: none;
            color: black;
            font-weight: bold;
        }

        h2 {
            align-self: center;
            margin-bottom: 20px;
        }

        .success-message {
            color: green;
            font-weight: bold;
            margin-top: 10px;
        }

        .error-message {
            color: red;
            font-weight: bold;
            margin-top: 10px;
        }
    </style>
</head>
<body>

    <h2>Book Vaccine</h2>
    <form action="booking.php" method="post">
        <label for="patient_id">Patient ID:</label>
        <input type="text" id="patient_id" name="patient_id" required>

        <!-- Add doctor_id input field -->
        <label for="doctor_id">Doctor ID:</label>
        <input type="text" id="doctor_id" name="doctor_id" required>

        <label for="vaccine_name">Vaccine Name:</label>
        <input type="text" id="vaccine_name" name="vaccine_name" required>

        <label for="dose">Dose:</label>
        <input type="text" id="dose" name="dose" required>

        <button type="submit">Book</button>
    </form>

    <?php
    if (isset($_POST['patient_id']) && isset($_POST['vaccine_name']) && isset($_POST['dose'])) {
        echo "<div class='success-message'>Vaccine successfully booked!</div>";
    }
    ?>

    <!-- Add a link to view all vaccine records (if needed) -->
   
</body>
</html>
